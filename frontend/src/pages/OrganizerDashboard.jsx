    ...
    <div className="min-h-screen bg-[#030712] flex items-center justify-center p-6">
      <div className="w-full max-w-2xl rounded-2xl border border-indigo-500/20 bg-white/5 backdrop-blur-sm p-10 text-center">
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          {['Create Event', 'View Events', 'Manage Stalls'].map((item) => (
            <div key={item} className="rounded-xl border border-indigo-500/20 bg-white/5 p-4 text-slate-300 text-sm font-medium hover:border-indigo-400 hover:text-white transition cursor-pointer">
              {item}
            </div>
          ))}
        </div>
